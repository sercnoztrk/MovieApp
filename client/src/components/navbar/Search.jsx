  ...
  return (
    <div className="relative flex flex-col pt-4 md:-pt-4  focus:outline-none focus:ring focus:ring-violet-300  z-50 ">
      <div className="flex z-50">
        <input onChange={(e) => setSearch(e.target.value)} className={`${styles.searchInput} `} type="text" placeholder="Type to search . . ." value={search}/>
        <RiSearch2Line onClick={() => setIsSearch(!isSearch)} className={`${styles.searchIcn}`}/>
      </div>
      <div className={`origin-top ${ (!isSearch || search === "" || search === " " || search === null) && "scale-y-0" } duration-500 absolute  top-12  w-full z-50 `}>
      </div>
